...
class Game:
    ...
    def reset_game(self):
        ...
        # PADDLE_SPEED se pasa como constante o valor fijo
        self.ball.serve(self.serve_direction, 300 * self.initial_ball_speed_options[self.initial_ball_speed_idx])
    ...
    def _handle_mouse_click(self, event):
        ...
        if self.state == STATE_MAIN_MENU:
            ...
            # Pestañas
                ...
                if self.modifiers_tab != "ALL": self.audio.play('pop'); self.modifiers_tab = "ALL"; self.scroll_y = 0
            elif self.tab_extras_rect.collidepoint(event.pos):
                if self.modifiers_tab != "EXTRAS": self.audio.play('pop'); self.modifiers_tab = "EXTRAS"; self.scroll_y = 0
            elif self.tab_skins_rect.collidepoint(event.pos):
                if self.modifiers_tab != "SKINS": self.audio.play('pop'); self.modifiers_tab = "SKINS"; self.scroll_y = 0
    ...
    def _handle_keydown(self, event):
        if self.state == STATE_PRESS_TO_START and event.key == pygame.K_SPACE:
            ...
            if event.key == pygame.K_d: self.paddle1.activate_power(self)
            if event.key == pygame.K_RIGHT: self.paddle2.activate_power(self)
    ...
    def check_collisions(self):
        ...
        if self.ball.rect.top <= 0:
            self.ball.rect.top = 0; self.ball.y_float = float(self.ball.rect.y); self.ball.vy = abs(self.ball.vy)
            if self.wall_sound_cooldown <= 0: self.audio.play('hit'); self.wall_sound_cooldown = 0.25
        elif self.ball.rect.bottom >= SCREEN_HEIGHT:
            self.ball.rect.bottom = SCREEN_HEIGHT; self.ball.y_float = float(self.ball.rect.y); self.ball.vy = -abs(self.ball.vy)
            if self.wall_sound_cooldown <= 0: self.audio.play('hit'); self.wall_sound_cooldown = 0.25

        # 3. Multiplicador X2
        if self.is_x2_item_active and self.ball.rect.colliderect(self.x2_item_rect):
            self.is_x2_item_active = False; self.ball_is_x2 = True; self.ball.color = GOLD; self.audio.play('x2')
        ...
        if self.ball.rect.right < 0:
            if self.ball.is_orange: self._handle_orange_bounce(True)
            elif self.paddle1.has_extra_life: self._handle_extra_life_save(True)
            else: self.goal_scored(2)
        elif self.ball.rect.left > SCREEN_WIDTH:
            if self.ball.is_orange: self._handle_orange_bounce(False)
            elif self.paddle2.has_extra_life: self._handle_extra_life_save(False)
            else: self.goal_scored(1)

        # 6. Paletas
        if self.ball.vx < 0 and self.ball.rect.colliderect(self.paddle1.rect): self.handle_paddle_collision(self.paddle1, 1)
        elif self.ball.vx > 0 and self.ball.rect.colliderect(self.paddle2.rect): self.handle_paddle_collision(self.paddle2, -1)
    ...
    def _check_planet_collisions(self):
        for p_idx, planet_pos in enumerate([self.planet1_pos, self.planet2_pos]):
            ...
            dx, dy = self.ball.rect.centerx - planet_pos[0], self.ball.rect.centery - planet_pos[1]
            dist = math.sqrt(dx**2 + dy**2)
            if dist < self.planet_radius + self.ball.rect.width / 2:
                nx, ny = dx/dist, dy/dist
                dot = self.ball.vx * nx + self.ball.vy * ny
                self.ball.vx -= 2 * dot * nx; self.ball.vy -= 2 * dot * ny
                overlap = (self.planet_radius + self.ball.rect.width / 2) - dist
                self.ball.rect.centerx += nx * (overlap + 2); self.ball.rect.centery += ny * (overlap + 2)
                self.ball.x_float = float(self.ball.rect.x); self.audio.play('pop')
    ...
    def _handle_watch_capture(self):
        ...
        if self.hourglass_type == 5:
            ...
            self.ball.is_orange = True; self.ball.color = ORANGE; self.ball.rect.width = self.ball.rect.height = BALL_SIZE * 2; self.ball.speed *= 1.5
        else:
    ...
    def _handle_orange_bounce(self, is_left):
        self.ball.rect.width = self.ball.rect.height = BALL_SIZE
        if is_left: self.ball.rect.left = 0
        else: self.ball.rect.right = SCREEN_WIDTH
        self.ball.vx *= -1; self.ball.speed /= 1.5; self.ball.is_orange = False; self.ball.color = WHITE; self.audio.play('hit')
        self.ball.x_float = float(self.ball.rect.x)

    def _handle_extra_life_save(self, is_left):
        p = self.paddle1 if is_left else self.paddle2
        if is_left: self.ball.rect.left = 0
        else: self.ball.rect.right = SCREEN_WIDTH
        self.ball.vx *= -1; p.has_extra_life = False; self.audio.play('hit')
        self.ball.x_float = float(self.ball.rect.x)

    def handle_paddle_collision(self, paddle, direction_x):
        if self.ball.is_orange:
            self.ball.reset_orange(BALL_SIZE); self.audio.play('pop')
        ...
        if self.ball.is_fireball:
            self.ball.is_fireball = False; self.reset_ball_visuals(); self.ball.speed /= 2; self.audio.fadeout('fire', 500)
        ...
        self._process_active_powers(paddle)
        self._calculate_bounce_physics(paddle, direction_x)
        self._check_watch_spawn()

    def _process_active_powers(self, paddle):
        if paddle.power_active == POWER_FIREBALL:
            paddle.power_active = POWER_NONE; paddle.color = WHITE; self.ball.is_fireball = True; self.ball.color = RED; self.ball.speed *= 2; self.audio.play('fire', loops=-1)
        elif paddle.power_active == POWER_ORANGE:
            paddle.power_active = POWER_NONE; paddle.color = WHITE; self.ball.is_orange = True
            if self.orange_skin_idx == 1: self.audio.play('hadouken')
            else: self.audio.play('uuui')
            self.ball.color = ORANGE if self.orange_skin_idx == 0 else (0, 150, 255)
            self.ball.rect.width = self.ball.rect.height = BALL_SIZE * 2; self.ball.speed *= 1.5
    ...
    def _calculate_bounce_physics(self, paddle, direction_x):
        rel_y = (paddle.rect.y + (paddle.rect.height / 2)) - self.ball.rect.centery
        norm_y = rel_y / (paddle.rect.height / 2)
        # MAX_BOUNCE_ANGLE se usa como constante o valor fijo
        angle = norm_y * math.radians(60) * -1
        self.ball.vx = self.ball.speed * math.cos(angle) * direction_x
        self.ball.vy = self.ball.speed * math.sin(angle)
        if direction_x == 1: self.ball.rect.left = paddle.rect.right
        else: self.ball.rect.right = paddle.rect.left
        self.ball.x_float = float(self.ball.rect.x)
    ...
    def reset_ball_visuals(self):
        self.ball.color = GOLD if self.ball_is_x2 else WHITE
    ...
    def goal_scored(self, player):
        ...
        if self.is_golden_goal_round:
            self.is_golden_goal_round = False
            msg = f"PLAYER {player} WINS!\n|YELLOW| (won by the 'Golden Goal' rule)"
        ...
        # Reset global round variables
        self.global_hits = 0; self.hourglass_rect = None; self.is_x2_item_active = False; self.pending_x2_spawn = False
        self.zone_type = self.slow_zone_owner = self.p1_zone_type = self.p2_zone_type = self.last_hitter = 0
        self.planet1_alive = self.planet2_alive = True; self.planet1_hits = self.planet2_hits = 0
        self.paddle1.reset(); self.paddle2.reset()
        if self.start_with_power_enabled: self.paddle1.grant_random_power(self); self.paddle2.grant_random_power(self)

        inc = 2 if self.ball_is_x2 else 1
        if player == 1: self.score1 += inc; s_dir = 1
        else: self.score2 += inc; s_dir = -1
        self.ball_is_x2 = False
        
        if self.score1 >= self.max_score or self.score2 >= self.max_score:
            if not self.match_point_enabled or abs(self.score1 - self.score2) >= 2:
                winner = 1 if self.score1 >= self.max_score else 2
                msg = f"PLAYER {winner} WINS!"
                if self.match_point_enabled:
                    msg += f"\n|YELLOW| (won by the 'Match Point' rule)"
                self.state = STATE_GAME_OVER; self.winner_text = msg
            else: self.state = STATE_SERVE; self.serve_direction = s_dir; self.ball.serve(s_dir, 300)
        else:
            self.state = STATE_SERVE; self.serve_direction = s_dir; self.ball.serve(s_dir, 300 * self.initial_ball_speed_options[self.initial_ball_speed_idx])
            if self.experimental_golden_goal and random.random() < 0.15: self.is_golden_goal_round = True
            if self.start_x2_enabled and random.random() < 0.50: self.pending_x2_spawn = True; self.is_golden_goal_round = False
    ...
    def _check_for_announcements(self):
        ...
        if self.match_point_enabled:
            ...
            if pts1 == 1 or pts2 == 1:
                self.show_match_point_anim = True; self.match_point_anim_timer = 2.1; self.serve_timer = 2.5; self.audio.play('match_point')
        
        if not self.show_match_point_anim:
            if not self.match_point_enabled and self.score1 == self.max_score-1 and self.score2 == self.max_score-1:
                self.show_golden_goal_anim = True; self.golden_goal_anim_timer = 2.1; self.serve_timer = 2.5; self.audio.play('golden_goal')
            elif self.is_golden_goal_round:
                self.show_golden_goal_anim = True; self.golden_goal_anim_timer = 2.1; self.serve_timer = 2.5; self.audio.play('golden_goal')
    ...
    def update(self, dt):
        ...
        if self.state == STATE_SERVE:
            ...
            self._apply_physics_effects(dt)
            self.ball.update(dt, self._get_zone_multiplier())
            self.check_collisions()
            
        self.vfx.update(dt)
        if self.state == STATE_PLAYING and self.ball.is_orange and self.orange_skin_idx == 1:
            self.vfx.trail(self.ball.rect.centerx, self.ball.rect.centery, (100, 200, 255))

    def _get_zone_multiplier(self):
        in_p1 = self.ball.rect.centerx < SCREEN_WIDTH // 2
    ...
    def _apply_physics_effects(self, dt):
        # Imán
        in_p1 = self.ball.rect.centerx < SCREEN_WIDTH // 2
        for i, p in enumerate([self.paddle1, self.paddle2]):
            ...
            if p.power_active == POWER_MAGNET and side:
                self._apply_magnet_force(p, dt)
            elif self.ball.color == (100, 200, 255) and not self.ball.is_fireball and not self.ball.is_orange:
                self.reset_ball_visuals()
        ...
        if self.floating_planets_enabled:
            ...
            for p_idx, pos in enumerate([self.planet1_pos, self.planet2_pos]):
                if (p_idx == 0 and self.planet1_alive) or (p_idx == 1 and self.planet2_alive):
                    dx, dy = pos[0]-self.ball.rect.centerx, pos[1]-self.ball.rect.centery
                    dist = math.sqrt(dx**2 + dy**2)
                    ...
                    if self.planet_radius < dist < rad:
                        ...
                        self.ball.vx += (dx/dist)*f*dt*60; self.ball.vy += (dy/dist)*f*dt*60
            new_spd = math.sqrt(self.ball.vx**2 + self.ball.vy**2)
            if new_spd > 0: self.ball.vx = (self.ball.vx/new_spd)*spd; self.ball.vy = (self.ball.vy/new_spd)*spd

    def _apply_magnet_force(self, p, dt):
        spd = math.sqrt(self.ball.vx**2 + self.ball.vy**2)
        incoming = (p == self.paddle1 and self.ball.vx < 0) or (p == self.paddle2 and self.ball.vx > 0)
        dx, dy = p.rect.centerx - self.ball.rect.centerx, p.rect.centery - self.ball.rect.centery
        dist = math.sqrt(dx**2 + dy**2)
        if dist > 0:
            target_vx, target_vy = (dx/dist)*spd, (dy/dist)*spd
            if incoming: self.ball.vx += (target_vx - self.ball.vx)*0.2*dt*60; self.ball.vy += (target_vy - self.ball.vy)*0.2*dt*60
            else: self.ball.vy += (target_vy - self.ball.vy)*0.1*dt*60
        new_spd = math.sqrt(self.ball.vx**2 + self.ball.vy**2)
        if new_spd > 0: self.ball.vx = (self.ball.vx/new_spd)*spd; self.ball.vy = (self.ball.vy/new_spd)*spd
    ...
    def draw(self):
        ...
        if self.state in [STATE_PLAYING, STATE_SERVE, STATE_PRESS_TO_START, STATE_GAME_OVER, STATE_MAIN_MENU]:
            ...
            if self.state in [STATE_PLAYING, STATE_SERVE]:
                if self.ball.is_orange and self.orange_skin_idx == 1:
                    pygame.draw.ellipse(temp_surf, (0, 150, 255), self.ball.rect)
                    pygame.draw.ellipse(temp_surf, WHITE, self.ball.rect.inflate(-6, -6))
                else: self.ball.draw(temp_surf)
            ...
            if self.state == STATE_SERVE:
                if self.show_match_point_anim and self.match_point_anim_timer > 0: self._draw_match_point_anim(temp_surf)
                if self.show_golden_goal_anim and self.golden_goal_anim_timer > 0: self._draw_golden_goal_anim(temp_surf)
    ...
    def _draw_score(self, surface):
        c1 = BLACK if (self.zone_type == 4 and self.slow_zone_owner == 1) else WHITE
        c2 = BLACK if (self.zone_type == 4 and self.slow_zone_owner == 2) else WHITE
    ...
    def _draw_match_point_anim(self, surface):
        t = 2.1 - self.match_point_anim_timer
        ...
        elif t < 1.3: cx = SCREEN_WIDTH//2 - tw//2
        else:
            t2 = (t-1.3)/0.8
    ...
    def _draw_golden_goal_anim(self, surface):
        if int(pygame.time.get_ticks() / 200) % 2 == 0:
            overlay = pygame.Surface((SCREEN_WIDTH, SCREEN_HEIGHT)); overlay.set_alpha(80); overlay.fill(GOLD); surface.blit(overlay, (0,0))
    ...
    def _draw_modifier_content(self, surface):
        ...
        if self.modifiers_tab == "ALL":
            ...
            self._draw_sub_selector(curr_y, "Ball speed increase:", self.ball_speed_multiplier_names[self.ball_speed_multiplier_idx], self.ball_speed_btn_rect, off, ox, lm, surface, sub_val=inc_val)
            ...
            curr_y += 80; draw_remove_option(self, curr_y, "Re-rolls:", self.reroll_enabled, self.reroll_rect, self.reroll_text_rect, offset=off, surface=surface)
            if self.reroll_enabled: curr_y += 80; draw_remove_option(self, curr_y, "All Re-roll:", self.all_reroll_enabled, self.all_reroll_rect, self.all_reroll_text_rect, offset=off, surface=surface)
            curr_y += 80; draw_remove_option(self, curr_y, "Equal watches:", self.equal_watches_enabled, self.equal_watches_rect, self.equal_watches_text_rect, offset=off, surface=surface)
            curr_y += 80; draw_remove_option(self, curr_y, "Equal powers:", self.equal_powers_enabled, self.equal_powers_rect, self.equal_powers_text_rect, offset=off, surface=surface)
    ...
    def _draw_sub_selector(self, y, label, val, rect, off, ox, lm, surface, sub_val=None):
        draw_rich_text(surface, label, (lm+30, self.modifiers_panel_rect.y + y - off), self.small_font)
    ...
    def _draw_game_over(self, surface):
        ...
        for i, line in enumerate(lines):
            # Usar draw_rich_text para la segunda línea con tiny_font y color persistente
            if "|" in line:
                # Calcular ancho real usando tiny_font (limpiando etiquetas para el cálculo)
                temp_txt = line.replace("|", "").replace("YELLOW", "").replace("RED", "").replace("BLUE", "")
                tw = self.tiny_font.render(temp_txt, True, WHITE).get_width()
                # Posicionamos la segunda línea un poco más abajo
                draw_rich_text(surface, line, (SCREEN_WIDTH//2 - tw//2, SCREEN_HEIGHT//3 + 55), self.tiny_font)
            else:
        ...
        # Recalcular rects para asegurar visibilidad
        self.btn_gameover_restart.update(SCREEN_WIDTH//2 - 150, SCREEN_HEIGHT//2 + 20, 300, 60)
        self.btn_gameover_menu.update(SCREEN_WIDTH//2 - 150, SCREEN_HEIGHT//2 + 100, 300, 60)
        
        for r, t in [(self.btn_gameover_restart, "Restart"), (self.btn_gameover_menu, "Menu")]:
            pygame.draw.rect(surface, BLACK, r); pygame.draw.rect(surface, WHITE, r, 4)
            st = self.font.render(t, True, WHITE); surface.blit(st, st.get_rect(center=r.center))
